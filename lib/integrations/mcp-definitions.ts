...
export const MCP_DEFINITIONS: McpDefinition[] = [
...

  {
    ...
    setupNotes:
      "Blotato is a unified publishing + visual-generation API for 9 social platforms (TikTok, Instagram, YouTube, X, LinkedIn, Threads, Facebook, Pinterest, Bluesky). Connect your social accounts in the Blotato dashboard first, then paste your API key below. **Two valid endpoints:** Ghost ProtoClaw agents call REST at https://backend.blotato.com/v2 (the API-key path); Blotato also exposes a native MCP server at https://mcp.blotato.com/mcp that you can connect from Claude.ai, Claude Desktop, Claude Code, Cursor, Antigravity, or Replit Agent — same account, same key, parallel access. The MCP endpoint supports OAuth (Claude.ai / Desktop / Cowork) OR API-key header authentication. **WARNING: api.blotato.com is NOT a valid base URL — it returns DNS errors.** All creation operations are async — agents submit a request, then poll status every 2-5 seconds for sources / posts and every 5 seconds for visuals. Failed statuses are PERMANENT — agents do not auto-retry. Free Trial includes API access; paid plans at https://blotato.com/pricing. Plan caps enforce queued-post limits (Starter 200 / Creator 1000 / Agency 3000) — exceeding returns HTTP 422 so agents self-throttle via the plan tier you select. For extending the integration beyond Ghost ProtoClaw's 20 wired tools, Blotato publishes a plain-text 'API Reference for LLMs' at https://help.blotato.com/api/llm.md plus 'Protocol and Recipes' at https://help.blotato.com/api/workflows.md.",
    setupSteps: [
      "Sign up at https://blotato.com. The Free Trial includes API access so you can test the integration before paying. Paid plans: Starter ~$33/mo (20 accounts, 200 queued posts, 9mo horizon), Creator (40 accounts, 1000 queued posts, 12mo horizon), Agency (100 accounts, 3000 queued posts, 24mo horizon).",
      ...
      "Open Settings → API in the Blotato dashboard, generate an API key, and paste it into the API Key field below. The key is sent on every request as the `blotato-api-key` header against https://backend.blotato.com/v2.",
      "Select your plan tier from the dropdown (including Free Trial) so agents self-throttle queued-post submissions before HTTP 422 fires.",
      ...
      "Click Test Connection — the agent calls blotato_get_user against https://backend.blotato.com/v2/users/me and confirms the API key works. On success the integration goes green and 21 blotato_* tools become available to every agent on templates that wire Blotato.",
      "Optional but recommended for first runs: open https://my.blotato.com/dashboard while the first cross-platform fan-out is running — every async submission shows up there in real time alongside any error message, which makes debugging the first attempt much faster than waiting on the agent's polling layer.",
      "Optional second hop: connect Blotato to your other MCP-compatible tools (Claude Desktop, Cursor, n8n, Make, Replit Agent) by pointing them at https://mcp.blotato.com/mcp. Same account, same scheduling queue — you can use the same Blotato workspace from agent runs in Ghost ProtoClaw and ad-hoc tasks in Claude Desktop simultaneously."
    ]
  },
