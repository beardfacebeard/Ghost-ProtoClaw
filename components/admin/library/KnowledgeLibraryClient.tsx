...
import type { KnowledgeLibraryItem } from "@/lib/library/types";
...
export function KnowledgeLibraryClient({
  ...
  async function install() {
    if (selected.size === 0) {
      toast.error("Pick at least one knowledge item to install.");
      return;
    }
    if (!targetBusinessId) {
    ...
    try {
      const response = await fetchWithCsrf(
        "/api/admin/library/knowledge/install",
        {
          method: "POST",
      ...
      toast.success(
        `Knowledge added to ${result.businessName}: ${
    ...
    <div className="space-y-4 pb-24">
      <div className="flex flex-wrap items-end gap-3">
        <div className="flex-1 min-w-[220px]">
          <Label htmlFor="kb-library-search" className="text-xs">
            Search
          </Label>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {filtered.map((item) => (
            <LibraryCard
              ...
              id={item.id}
              title={item.title}
              subtitle={formatKnowledgeCategory(item.category)}
              description={
                item.description ??
